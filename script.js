        ...
        snapshot.docChanges().forEach((change) => {
            if (change.type === 'modified') {
                ...
                if (categoryData.lastSaveNotification) {
                    ...
                    // Chỉ hiển thị nếu:
                    // 1. Notification mới (trong vòng 5 giây)
                    // 2. Không phải category đang mở
                    // 3. User đã bật notification
                    if ((now - notifTime) < 5 &&
                        (!currentCategory || currentCategory.id !== change.doc.id) &&
                        notificationEnabled) {
                        
                        // Hiển thị thông báo
                        showDataUploadNotification(
...
async function saveData() {
    try {
        ...
        // Cập nhật notification cho category
...
async function initializeApp() {
    try {
        ...
        // Load notification state (SIMPLE VERSION - NO PUSH)
...
function selectRole(role) {
    try {
        ...
        // Với vai trò inventory
        currentRole = role;
        document.body.classList.add('role-inventory');
...
// ==================== SIMPLE NOTIFICATION SYSTEM (NO PUSH) ====================

let notificationEnabled = false;

// Load trạng thái notification từ localStorage
...
// Xử lý toggle notification - ĐƠN GIẢN
...
// Hiển thị thông báo trên web (NO PUSH NOTIFICATION)
function showDataUploadNotification(categoryName, itemCount) {
    ...
    notificationDiv.innerHTML = `
        ...
        <div class="notification-content">
            <strong>📋 ${categoryName}</strong>
            <span>Đã có ${itemCount} dòng dữ liệu mới. Đã đến lúc kiểm kê!</span>
        </div>
    `;
...
// Hàm phát âm thanh thông báo (giữ nguyên)
function playNotificationSound() {
    try {
        const AudioContext = window.AudioContext || window.webkitAudioContext;
        const audioContext = new AudioContext();
...
// ==================== END SIMPLE NOTIFICATION SYSTEM ====================
